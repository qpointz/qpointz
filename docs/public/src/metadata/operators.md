...
- **`mill.metadata.seed.resources`** — **Only** supported startup path for loading seed YAML: global scope, standard facet types, and any follow-on datasets. List **`classpath:metadata/platform-bootstrap.yaml` first** in production so the global scope and built-in facet types exist (Flyway creates metadata **tables only** — no data inserts). Each entry is a **Spring resource location** (`classpath:`, `file:`, `s3://`, `gs://`, `azure-blob://`, …); entries are imported at startup in **MERGE** mode with actor `system`. Do **not** put secrets in URIs — use `mill.cloud.aws.s3.*`, `mill.cloud.gcp.gcs.*`, or `mill.cloud.azure.adls.*`, or ambient SDK credentials (see [Cloud resource loading](../reference/cloud-resource-loading.md)). **One auth profile per provider** applies to every cloud seed URL; if flow descriptors also use `s3://` URLs, they share the same S3 client settings. Seeds in bucket A and flow config in bucket B with **different** S3 key pairs are not supported unless one IAM identity can read both. An empty list means the seed runner applies no files (metadata stays empty until API/import unless you rely on pre-existing DB rows). When JPA and a seed ledger are present, completion is recorded (see design SPEC §14.1). The ledger **`seed_key`** stays stable when you reorder the list if keys are derived from canonical `file:` URIs or provider-stable keys; **MD5** of the raw bytes detects content changes.
