...
| 5 | **`mill.ai.value-mapping.refresh.*`** implemented with **locked** types (**§ Configuration** — **`scheduleInterval`** = **`Duration`**). |
...
|----------|---------|---------|
| **`startup-enabled`** | Global gate: when **false**, **no** **`APP_STARTUP`** refresh runs (facet flags ignored for startup). | **`true`** |
| **`scheduled-disabled`** | When **true**, **no** **`SCHEDULED_TICK`** runs (orchestrator does not register the scheduler). When **false**, scheduled ticks are **active**. | **`false`** |
| **`schedule-interval`** | Fixed cadence for **how often** the **scheduled** job **wakes** to **evaluate** due attributes (not the same as per-facet **`refreshInterval`**). | **`15m`** |
...
| **Java field** | **`scheduleInterval`** on a nested **`Refresh`** type (or equivalent) under **`ValueMappingConfigurationProperties`** — type **`java.time.Duration`**, default **`Duration.ofMinutes(15)`**. |
...
- **`scheduled-disabled=true`** disables the scheduler; **`startup-enabled=false`** skips startup pass.
