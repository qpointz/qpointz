...
>
> **Implementation note:** Facet types are deployment-configurable (dynamic), so the UI editing
> experience must be schema-driven from `FacetTypeDescriptor.contentSchemaJson`, not a fixed enum.
...
- `setFacet(entityId, facetType, scopeKey, data, actorId)` — validate against
  `FacetTypeDescriptor.contentSchemaJson`, check scope ownership, save, emit `FacetUpdated`.
- `deleteFacet(entityId, facetType, scopeKey, actorId)` — remove scope entry, emit `FacetDeleted`.
- Throws `MillStatuses.notFoundRuntime`, `MillStatuses.forbiddenRuntime`,
  `MillStatuses.unprocessableRuntime`.
