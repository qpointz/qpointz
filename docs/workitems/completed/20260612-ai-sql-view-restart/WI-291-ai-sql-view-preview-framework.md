...
- [x] Framework compiles and unit tests pass in isolation.
- [x] `SqlDataCondensedPreview` renders SQL from `Message.artifacts` (mock/fixture).
- [x] Run calls `queryService` + attach API (mocked).
- [x] Facet/schema/unknown route through existing `ArtifactCard` components.
- [x] No salvage symbols in new files.
