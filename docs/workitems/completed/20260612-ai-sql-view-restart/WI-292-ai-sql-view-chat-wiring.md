...
- [x] `general` `/chat`: live structured SQL (from coordinator) → Run → Data in message state.
- [x] `inline-analysis`: SQL updates editor; drawer has **no** `SqlDataCondensedPreview`.
- [x] `inline-model` / `inline-knowledge`: `ArtifactCard` for facet/schema/unknown.
- [x] GET hydrates `Message.artifacts` on all chat types.
- [x] No client salvage code paths.
