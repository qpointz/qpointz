...
| `getSavedQueries()` | `GET /api/v1/queries` |
| `getSavedQueryById(id)` | `GET /api/v1/queries/{id}` |
...
5. **Cleanup (same WI or follow-up note in commit):**
   ...
   - **`searchService`** / **`statsService`** still import `mockSavedQueries` today — either switch those to **`GET /api/v1/queries`** in this WI or leave a short WI note; prefer aligning search/stats query counts with the live catalog when backend is up.
...
- `queryService` has **no mock code path**; Analysis requires reachable **`/api/v1/queries`** and **`/api/v1/query`** (dev: Vite proxy to `mill-service`).
