...
Persist **saved queries**, expose the Analysis HTTP surface under **`/api/v1/analysis/**`** (dialect + saved-query catalog CRUD) per [`BACKEND-API-REQUIREMENTS.md`](../../../design/ui/mill-ui/BACKEND-API-REQUIREMENTS.md). Refactor **[`queryService.ts`](../../../../ui/mill-ui/src/services/queryService.ts)** to **HTTP only** — catalog and execution use **`fetch`** with **`credentials: 'include'`** against **`/api/v1/analysis/**`** and **`/api/v1/query/**`** (**not** `POST /api/v1/queries/execute`). Analysis SQL editing uses **CodeMirror 6** (`SqlCodeEditor`) with **schema-linked** completions.
...
- [`analysis-saved-query-service.md`](../../../design/platform/analysis-saved-query-service.md) — Gradle modules and REST catalogue
- [`BACKEND-API-REQUIREMENTS.md`](../../../design/ui/mill-ui/BACKEND-API-REQUIREMENTS.md) — Analysis: catalog, **`/api/v1/query/**`**, **`/services/export/**`**
...
- [x] WI-258 — Analysis HTTP module + `mill-service` wiring (`WI-258-analysis-service-module-wiring.md`)
- [x] WI-257 — REST `/api/v1/analysis/queries` catalog (`WI-257-analysis-queries-rest-api.md`)
