...
2. **`CliApp` / `SchemaExplorationAgent.fromEnv`:** pass a **`SchemaCatalogPort`** built from **`SchemaFacetService`** using the WI-162 factory (e.g. **`demoSchemaFacetService().asSchemaCatalogPort()`**) — **manual** glue acceptable here only because the REPL is not a Spring app.
