...
3. **(B)** Implement **`SchemaFacetCatalogAdapter`** (or equivalent) implementing **`SchemaCatalogPort`**, by **moving** the former **`SchemaToolHandlers`** body from **`mill-ai-v3`** into this module (same behaviour, **`SchemaFacetService`** as collaborator).
4. Provide a small **factory** or extension such as **`SchemaFacetService.asSchemaCatalogPort()`** for **non-Spring** call sites (tests, **temporary** CLI glue — **WI-163**). **Spring Boot** hosts should prefer the bean registered in **`mill-ai-v3-autoconfigure`** (**WI-164**).
