...
> the review UI is surfaced. These decisions depend on the scope permission model (WI-089) being
> live and on team/role administration being defined. Implement after WI-089 and WI-090 are
> complete and the workflow state machine is agreed.
...
2. Conflict behaviour: target scope already has a different value — auto-overwrite or require explicit `force`?
