...
|-------|-----------|--------|--------|
| (core) | `pip install qpointz-mill-py` | `result.fetchall()` | `list[dict]` |
| arrow | `pip install qpointz-mill-py[arrow]` | `result.to_arrow()` | `pyarrow.Table` |
| pandas | `pip install qpointz-mill-py[pandas]` | `result.to_pandas()` | `pandas.DataFrame` |
| polars | `pip install qpointz-mill-py[polars]` | `result.to_polars()` | `polars.DataFrame` |
...
7. **Mitigated on data-plane HTTP/JDBC/Python** (`AccessServiceProblemAdvice`, `HttpMillErrorBodies`, enriched `mill-py` mappers) — see [`client-error-transparency.md`](./client-error-transparency.md). Remaining content-negotiation/decoder mismatch risk tracked as `C-20` above when mixing JSON vs protobuf HTTP modes.
...
- Should the ibis backend ship as `qpointz-mill-py[ibis]` or as a separate package?
...
- Should the SQLAlchemy dialect ship as `qpointz-mill-py[sqlalchemy]` or a separate package?
...
| **Lazy imports for extras** | `to_arrow()`, `to_pandas()`, `to_polars()` must use lazy imports. If the extra is not installed, raise `ImportError` with install instructions (e.g. `"Install qpointz-mill-py[arrow]"`). Never import `pyarrow`, `pandas`, or `polars` at module top-level in core code. |
