...
| `local-dev-authentik.md` | Local **Authentik** OIDC IdP in `deploy/local-dev`: compose (server + worker), Postgres DB, blueprints under `/blueprints/custom`, OIDC URLs for Spring Boot, HTTP vs HTTPS |
