...
| `:core:mill-events` | Kotlin | pure | Mill application event bus contracts: `Event`, `EventPublisher`, `EventTransport`, `EventRouter`, per-type subscriptions | none (foundation module) | none | `true` | JUnit 5 |
| `:core:mill-events-autoconfigure` | Kotlin | spring-service | Event bus Spring wiring, `mill.events.*` properties, transport bean selection | `:core:mill-events` | Spring Boot autoconfiguration | `true` | JUnit 5 + Boot test + testIT |
