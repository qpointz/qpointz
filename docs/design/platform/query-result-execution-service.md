...
- **Programmatic-first:** a Spring-free core (**`QueryResultExecutionService`**) owns **execution sessions**: SQL (or plan reference per core API), **paged** reads over **`VectorBlock`** via **`DataOperationDispatcher`** (`submitQuery` / `fetchResult` / `execute`), optional **marshaller** sinks, **Caffeine** idle eviction, and **`PUT`** replace on the **same** opaque **`executionId`**.
...
- **Presentation vs engine window:** clients use **`pageIndex`** / **`pageSize`** on **`GET …/rows`**. Internally the core may use **`executionBufferRows`**, **`backwardCacheBuffers`**, **`forwardCacheBuffers`**, full snapshot, or refill-on-miss — those are **implementation details** behind the paging envelope.
...
- **No** public **`offset` / `limit`** on this tree; **only** **`pageIndex`** / **`pageSize`** for **`GET …/rows`**.
...
On **every** read/write (**`GET` metadata**, **`GET …/rows`**, **`PUT` replace**, **`DELETE`**), the core compares **current request tenant** to **stored tenant**.
...
| **`GET`** | `/api/v1/query/{executionId}` | Metadata: schema hints, truncation, **`epoch`**, **`totalResult`**, marshaller hints, defaults. |
| **`GET`** | `/api/v1/query/{executionId}/rows` | One presentation page; query **`pageIndex`**, **`pageSize`**; optional **`format`**, **`epoch`**. |
| **`PUT`** | `/api/v1/query/{executionId}` | Replace SQL/plan for same **`executionId`**; tear down prior buffers; **`epoch`** increments on success. |
...
## Paging contract (`GET …/rows`)

**Canonical request:** **`GET /api/v1/query/{executionId}/rows?pageIndex=&pageSize=`** (optional **`format`**, **`epoch`**).

**Envelope** (for **`application/json`** responses: paging fields at top level; marshaller payload under **`data`**):

| Field | Purpose |
|--------|---------|
| **`epoch`** | Session generation: **`0`** at create, **`+1`** each successful replace. Echoed on **`/rows`** and metadata. |
...
**Caffeine** **`expireAfterAccess`** and **`replace` / `DELETE`** drop server buffers; clients must not assume backward stays cheap after idle or eviction without a new **`POST`**.
...
**JSON shape:** one object: envelope fields + **`data`** for marshaller body.
...
| **`POST`** create **with** first **`/rows`**-shaped page in same document | **`200 OK`** |
| **`DELETE`** success | **`204 No Content`**, **no** body |
| Malformed JSON, bad **`pageIndex`/`pageSize`**, unknown **`format`**, contract violations | **`400`** |
| SQL / plan fails but request structurally valid (auth OK, limits OK) | **`422 Unprocessable Entity`** |
| **`GET …/rows?epoch=`** stale vs server session | **`409 Conflict`** (structured error **`code`**) |

**`PUT` replace** uses the same **`422`** vs **`400`** split for SQL/plan failures.

---

## Concurrency, replace, and consistency

- **`epoch`:** monotonic per **`executionId`**; metadata and **`/rows`** include current value.
- **Optional `epoch` on `GET …/rows`:** mismatch ⇒ **`409`**. If **`epoch`** omitted, serve current epoch; **`replace`** does not cause **`409`** by itself because reads and replace are **serialized**.
- **`replace` vs reads:** **read–write ordering** — **`replace`** waits for in-flight reads; new reads block until **`replace`** completes. After **`replace`**, no **`/rows`** payload may come from **pre-replace** buffers (implementation: per-session **`ReentrantReadWriteLock`** or equivalent).
...
| [../../workitems/in-progress/query-result-execution-service/STORY.md](../../workitems/in-progress/query-result-execution-service/STORY.md) | WI checklist, cold start, story closure table |
