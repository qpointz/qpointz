...
| `mill.cloud.aws.s3.endpoint` / `mill.cloud.aws.s3.region` / `mill.cloud.aws.s3.access-key` / `mill.cloud.aws.s3.secret-key` | Optional S3 client overrides for `s3://` seed and flow resources (MinIO, static credentials). | `mill-cloud-aws-autoconfigure`, IT |
| `mill.cloud.gcp.gcs.emulator-host` / `mill.cloud.gcp.gcs.project-id` | Optional GCS emulator base URL and project id for `gs://` resources. | `mill-cloud-gcp-autoconfigure`, IT |
| `mill.cloud.azure.adls.connection-string` / `mill.cloud.azure.adls.blob-service-endpoint` | Azure Blob connection string or HTTPS service endpoint for `azure-blob://` resources. | `mill-cloud-azure-autoconfigure`, IT |
