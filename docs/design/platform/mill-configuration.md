...
| `mill.metadata.seed.resources`                                                                                              | **Only** startup seed pipeline: ordered canonical YAML (typically `classpath:metadata/platform-bootstrap.yaml` first, then env-specific files). Flyway does not insert metadata rows. Each entry is a Spring resource location (`classpath:`, `file:`, `s3://`, `gs://`, `azure-blob://`). | `apps/mill-service`, metadata testIT                                                                   |
| `mill.cloud.aws.s3.endpoint` / `mill.cloud.aws.s3.region` / `mill.cloud.aws.s3.access-key` / `mill.cloud.aws.s3.secret-key` | **Single** S3 client profile for **all** `s3://` config URLs (metadata seeds + flow descriptor paths). Bucket is only in the URI. Not per-bucket auth. Query data auth remains in each descriptor `storage.auth`. See `docs/design/platform/cloud-resource-loading.md`.                    | `mill-cloud-aws-autoconfigure`, IT                                                                     |
| `mill.cloud.gcp.gcs.emulator-host` / `mill.cloud.gcp.gcs.project-id`                                                        | **Single** GCS client profile for **all** `gs://` config URLs (same limitation as S3).                                                                                                                                                                                                     | `mill-cloud-gcp-autoconfigure`, IT                                                                     |
| `mill.cloud.azure.adls.connection-string` / `mill.cloud.azure.adls.blob-service-endpoint`                                   | **Single** Azure Blob client profile for **all** `azure-blob://` config URLs.                                                                                                                                                                                                              | `mill-cloud-azure-autoconfigure`, IT                                                                   |
...
**Summary:** sync / embed / vector-store paths from [**WI-179**](../../workitems/completed/20260416-implement-value-mappings/WI-179-sync-vectors-hydration.md) / [**WI-180**](../../workitems/completed/20260416-implement-value-mappings/WI-180-value-mapping-service-orchestrator.md) need **Micrometer** (or platform-standard) metrics for production visibility; **not** in the delivered story’s acceptance criteria — follow-up tracking: [`docs/workitems/completed/20260416-implement-value-mappings/STORY.md`](../../workitems/completed/20260416-implement-value-mappings/STORY.md).
