...
| Execute action | Runs query | `queryService.executeQuery()` → **`POST /api/v1/query`** + paged **`GET /api/v1/query/{executionId}?pageIndex=…`**; **`DELETE`** session in `finally` | `analysisExecuteQuery` | `true` | real |
