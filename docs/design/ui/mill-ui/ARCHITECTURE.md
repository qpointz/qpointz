...
│   │   ├── facetPayloadUtils.ts   # Pure MULTIPLE facet payload normalise/replace/append (WI-110)
...
### Explorer split layout (Model, Knowledge, Analysis)

**Files:** `src/components/layout/layoutChrome.ts`, `ViewPaneHeader.tsx`, `ExplorerSplitLayout.tsx`

Two-row shell under `AppHeader` (WI-107):

1. **Toolbar row** — fixed height `PANE_TOOLBAR_HEIGHT` (44px), shared bottom border: left cell `SIDEBAR_WIDTH` (280px) with sidebar title + optional `sidebarHeaderRight` + collapse; right cell `ViewPaneHeader` for route-specific actions (e.g. metadata **scope** + unbound-metadata preview on Model).
2. **Body row** — sidebar tree (scroll) + main pane.

`SIDEBAR_WIDTH` is the same constant as the brand column in `AppHeader` and the offset used by `GlobalSearch`.

**Chat** still uses `CollapsibleSidebar` only (no shared view-pane header row).

### CollapsibleSidebar

**File:** `src/components/common/CollapsibleSidebar.tsx`

Reusable sidebar wrapper used by **Chat** (`AppShell`) and any layout not yet on `ExplorerSplitLayout`. Model / Knowledge / Analysis use `ExplorerSplitLayout` instead.
