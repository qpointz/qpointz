...
| **Replay** | `GET /api/v1/ai/chats/{id}` → `TurnResponse` list | Durable `text` per turn; `artifacts[]` wire payloads (`sql`, `data`, `facet-proposal`); optional `assistantReplyView`. Client: [`parseWireArtifacts`](../../../ui/mill-ui/src/utils/artifactWireParse.ts). See [`chat-artefact-architecture.md`](../ai/chat-artefact-architecture.md). |
