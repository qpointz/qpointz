...
| `mill.application.hosts.externals.<name>.scheme` | String | `ServiceAddressProperties` (`@ConfigurationProperties` prefix `mill.application.hosts`) | services:mill-service-common | `GrpcConnectionDescriptor`, `HttpConnectionDescriptor` via `ExternalHostsProvider` | **mill-service** | Literal (`http`, `https`, `grpc`) or placeholder `@request.scheme` (quote in YAML) |
| `mill.application.hosts.externals.<name>.host` | String | `ServiceAddressProperties` | services:mill-service-common | Same as above | **mill-service** | Literal host or `@request.host` |
| `mill.application.hosts.externals.<name>.port` | String | `ServiceAddressProperties` | services:mill-service-common | Same as above | **mill-service** | Literal port (e.g. `9090`) or `@request.port`; numeric YAML still binds as string |
...
| `mill.data.query.*` | *(nested)* | `MillDataQueryProperties` | data:mill-data-autoconfigure | **`QueryResultEngineAutoConfiguration`** → **`QueryResultExecutionService`** `@Bean` | **mill-data-autoconfigure** | **`enabled`**, **`max-materialized-rows`** (per-scan cap), **`max-cached-pages`** (backward window), session idle expiry, **`max-page-size`**, fetch size — **no** Spring inside **`mill-data-query`**; see [`query-result-execution-service.md`](../platform/query-result-execution-service.md). |
