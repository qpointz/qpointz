...
| `mill.data.query.*` | *(nested)* | `MillDataQueryProperties` | data:mill-data-autoconfigure | **`QueryResultEngineAutoConfiguration`** → **`QueryResultExecutionService`** `@Bean` | **mill-data-autoconfigure** | Core session tuning + **`enabled`** flag; **no** Spring inside **`mill-data-query`**; see [`query-result-execution-service.md`](../platform/query-result-execution-service.md). |
