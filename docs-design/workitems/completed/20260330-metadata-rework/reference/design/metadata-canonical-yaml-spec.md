...
**Normative implementation (today):** `metadata/mill-metadata-service/.../DefaultMetadataImportService.kt`  
**Domain types:** `metadata/mill-metadata-core/.../MetadataEntity.kt`, `FacetTypeManifest.kt`, `MetadataUrns.kt`, `MetadataEntityUrn.kt`  
**Default platform facet manifests (source of truth):** `metadata/mill-metadata-core/src/main/resources/metadata/platform-facet-types.json` — summarised in §11.
...
- **Empty or absent:** allowed. If **no** document in the stream provides a non-empty `facet-types` list, the importer **ensures platform facet types** exist in `FacetTypeRepository` (`ensurePlatformFacetTypesPresent`). Those definitions match the JSON array in `platform-facet-types.json`; see §11 for each type’s title, applicability, and payload fields.
...
5. If no facet-types were supplied in the entire stream, ensure platform facet types exist (mirror `ensurePlatformFacetTypesPresent`).
...
## 11. Default platform facet types

**Path:** `metadata/mill-metadata-core/src/main/resources/metadata/platform-facet-types.json`

The resource is a JSON array of **four** manifests (`PlatformFacetTypeDefinitions`); export/import treat these URNs as non-custom. `MetadataUrns` also recognises short keys such as `concept` and `value-mapping` for normalisation, but **they are not defined in this file** — only the types below are seeded from it.

This file is the **authoritative manifest list** for the built-in facet types Mill seeds when YAML does not declare `facet-types`. Each entry is a `FacetTypeManifest`-compatible object (`typeKey`, `title`, `description`, `category`, `enabled`, `mandatory`, `targetCardinality`, `applicableTo`, `schemaVersion`, `payload`).
...
### 11.1 `urn:mill/metadata/facet-type:descriptive` — Description
...
### 11.2 `urn:mill/metadata/facet-type:source-table` — Source Table
...
### 11.3 `urn:mill/metadata/facet-type:source-column` — Source Column
...
### 11.4 `urn:mill/metadata/facet-type:relation` — Relation
...
*Last updated: aligns with Mill `DefaultMetadataImportService` behaviour as of repository snapshot; when behaviour changes, update this file and reference tests under `metadata/mill-metadata-service/src/test/…`.*
