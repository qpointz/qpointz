"""Asynchronous schema explorer HTTP client (``/api/v1/schema``)."""
...
from mill._http_common import build_platform_async_client, raise_for_status
...
class AsyncSchemaExplorerClient:
    """Read-only schema + merged-facets explorer."""

    def __init__(self, http: httpx.AsyncClient, *, prefix: str = "/api/v1/schema") -> None:
        self._http = http
    ...
    async def __aenter__(self) -> AsyncSchemaExplorerClient:
        return self
    ...
    async def get_context(self) -> SchemaContextDto:
        """``GET …/context``."""
...
async def connect(
    ...
    *,
    ...
    timeout: float = 30.0,
) -> AsyncSchemaExplorerClient:
    """Factory: new :class:`httpx.AsyncClient` + :class:`AsyncSchemaExplorerClient`."""
    ...
    return AsyncSchemaExplorerClient(http, prefix=prefix)
