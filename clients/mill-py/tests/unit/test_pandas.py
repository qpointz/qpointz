"""Tests for mill.extras.pandas — ResultSet to pandas.DataFrame."""
...
class TestResultToPandas:
    """Validate pandas DataFrame conversion."""
    ...
    def test_null_values(self) -> None:
        ...
        assert df["x"].iloc[2] == 30

    def test_to_pandas_method(self) -> None:
        block = _make_block([("s", MillType.STRING.proto_id)], [_string_vector(0, ["hello"])], 1)
        rs = _mock_result_set([block])
        df = rs.to_pandas()
        assert isinstance(df, pd.DataFrame)
...
@pytest.mark.unit
class TestAllTypesPandas:
    """Verify pandas conversion for every Mill logical type."""
