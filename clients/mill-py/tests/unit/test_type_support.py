    ...
    def parse(self):
        ...
        match self.type:
            ...
            case LogicalDataTypeLogicalDataTypeId.DATE:
                return PlainDateTime.parse_strptime(self.value, format="%Y-%m-%d").date()
            case LogicalDataTypeLogicalDataTypeId.TIME:
                ldt = PlainDateTime.parse_strptime(self.value[:-10], format="%H:%M:%S")
            ...
            case LogicalDataTypeLogicalDataTypeId.TIMESTAMP_TZ:
                ldt = PlainDateTime.parse_strptime(self.value[:-10], format="%Y-%m-%d %H:%M:%S")
                nanos = int(self.value[-9:])
