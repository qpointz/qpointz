...
/**
  * This provides an event sourced behavior. It has a state, [[SurfaceState]], which
  ...
  */
object SurfaceBehavior {

  /**
    * Given a sharding [[EntityContext]] this function produces an Akka [[Behavior]] for the aggregate.
    */ 
  def create(entityContext: EntityContext[SurfaceCommand]): Behavior[SurfaceCommand] = {
    ...
    create(persistenceId)
      .withTagger(
        ...
        AkkaTaggerAdapter.fromLagom(entityContext, SurfaceEvent.Tag)
      )
  ...
  private[impl] def create(persistenceId: PersistenceId) = EventSourcedBehavior
      .withEnforcedReplies[SurfaceCommand, SurfaceEvent, SurfaceState](
        persistenceId = persistenceId,
        emptyState = SurfaceState.initial,
...
/**
  ...
  */
case class SurfaceState(message: String, timestamp: String) {
  def applyCommand(cmd: SurfaceCommand): ReplyEffect[SurfaceEvent, SurfaceState] =
    cmd match {
      case x: Hello              => onHello(x)
      case x: UseGreetingMessage => onGreetingMessageUpgrade(x)
    }

  def applyEvent(evt: SurfaceEvent): SurfaceState =
    evt match {
      case GreetingMessageChanged(msg) => updateMessage(msg)
    }
  private def onHello(cmd: Hello): ReplyEffect[SurfaceEvent, SurfaceState] =
    Effect.reply(cmd.replyTo)(Greeting(s"$message, ${cmd.name}!"))

  private def onGreetingMessageUpgrade(
    cmd: UseGreetingMessage
  ): ReplyEffect[SurfaceEvent, SurfaceState] =
    Effect
...
object SurfaceState {
  ...
  /**
    ...
    */
  def initial: SurfaceState = SurfaceState("Hello", LocalDateTime.now.toString)
  ...
  /**
    ...
    */
  val typeKey = EntityTypeKey[SurfaceCommand]("SurfaceAggregate")
  ...
  /**
    ...
    */
  implicit val format: Format[SurfaceState] = Json.format
}

/**
  * This interface defines all the events that the SurfaceAggregate supports.
  */
sealed trait SurfaceEvent extends AggregateEvent[SurfaceEvent] {
  def aggregateTag: AggregateEventTag[SurfaceEvent] = SurfaceEvent.Tag
}

object SurfaceEvent {
  val Tag: AggregateEventTag[SurfaceEvent] = AggregateEventTag[SurfaceEvent]
}

/**
  * An event that represents a change in greeting message.
  */
case class GreetingMessageChanged(message: String) extends SurfaceEvent
...
/**
  ...
  */
trait SurfaceCommandSerializable

/**
  * This interface defines all the commands that the SurfaceAggregate supports.
  */
sealed trait SurfaceCommand
    extends SurfaceCommandSerializable
...
/**
  ...
  * application loader.
  */
object SurfaceSerializerRegistry extends JsonSerializerRegistry {
  override def serializers: Seq[JsonSerializer[_]] = Seq(
    // state and events can use play-json, but commands should use jackson because of ActorRef[T] (see application.conf)
    JsonSerializer[GreetingMessageChanged],
    JsonSerializer[SurfaceState],
