...
@JsonSubTypes(
    ...
    JsonSubTypes.Type(value = JsonNodeJsonPathAssert::class, name = "json-path"),
)
...
data class JsonNodeJsonPathAssert(val exp: String) : JsonNodeAssert, JsonPathMatcher {
    override fun assert(node: JsonNode?) {
        assertTrue(node?.matches(exp) == true, "Expected JSONPath '$exp' to match node")
    }
}
