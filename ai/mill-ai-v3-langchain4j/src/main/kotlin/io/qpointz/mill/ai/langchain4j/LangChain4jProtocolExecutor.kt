...
import io.qpointz.mill.ai.ProtocolExecutionResult
import io.qpointz.mill.ai.ProtocolExecutor
...
class LangChain4jProtocolExecutor(
    ...
    private fun executeStructuredFinal(
        ...
        val jsonSchema = JsonSchema.builder()
            ...
            .rootElement(ToolSchemaConverter.toJsonObjectSchema(finalSchema))
            .build()
        ...
        val payload = future.join()
        // Validate: must be parseable JSON
        objectMapper.readTree(payload)
        input.listener(AgentEvent.ProtocolFinal(protocolId = input.protocol.id, payload = payload))
        return ProtocolExecutionResult(text = payload, payload = payload)
    }
    ...
    private fun executeStructuredStream(
        ...
        val buffer = JsonlLineBuffer(
            onLine = { line ->
                ...
                val content = node.path("content").toString()
                emittedPayloads.add(content)
                input.listener(
