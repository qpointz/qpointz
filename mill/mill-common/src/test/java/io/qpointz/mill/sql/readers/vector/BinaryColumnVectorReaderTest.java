...
class BinaryColumnVectorReaderTest {
    ...
    private final byte[] a1 = new byte[] {100, 12, -100, 12, (byte)234, (byte)-234};
    private final byte[] a2 = new byte[] {};
    private final byte[] a3 = Base64.getDecoder().decode("3wWJVd0GE1CELMW6eiBFa2irYE3cLZIU0Z2Kx+yNctvVQaFcbPLhJXVGgzHP7Y/3zWbmkeMUAOvi" +
    ...
    private final Vector vector =new TestVectorData.LogicalTypeVectorData<>("Binary", BinaryLogical.INSTANCE, TestVectorData::bytesToString,
            new byte[][] {
            a1,
            null,
            a2,
    ...
    private final BinaryColumnVectorReader reader = new BinaryColumnVectorReader(vector);
    ...
    @Test
    void readBytesStream() throws IOException {
        val buff = new byte[a1.length];
        reader.getBinaryStream(0).read(buff);
