...
@Builder
public class MillConnectionConfig {
    ...
    private static MillConnectionConfig from(Properties props) {
        return MillConnectionConfig.builder()
                .host(props.getOrDefault("host", "").toString())
                .port((int)props.getOrDefault("port", 0))
                .protocol((String)props.getOrDefault("protocol", "grpc"))
                .build();
    }
    public static MillConnectionConfig from(String url) {
        return from(url, new Properties());
    }

    public static MillConnectionConfig from(String url, Properties info) {
        ...
        return MillConnectionConfig.from(effectiveProps);
    }
