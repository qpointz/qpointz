...
class AggregationPhase(betterproto.Enum):
    ...
    intermediate values of the function, and that the output should also be an
    intermediate value, to be further aggregated later using
    INTERMEDIATE_TO_INTERMEDIATE or INTERMEDIATE_TO_RESULT.
    """
    ...
    This call should combine the intermediate values to yield the final return
    value.
    """
...
class JoinRelJoinType(betterproto.Enum):
    ...
    JOIN_TYPE_SINGLE = 7
    """
    This join is useful for nested sub-queries where we need exactly one record
    in output (or throw exception) See Section 3.2 of https://15721.courses.cs.
    cmu.edu/spring2018/papers/16-optimizer2/hyperjoins-btw2017.pdf
    """
...
class DdlRelDdlOp(betterproto.Enum):
    ...
    """
    A create operation if the object does not exist, or replaces it (equivalent
    to a DROP + CREATE) if the object already exists
    """

    DDL_OP_ALTER = 3
    """
    An operation that modifies the schema (e.g., column names, types, default
    ...
    """
    An operation that removes an object from the system (without throwing an
    exception if the object did not exist)
    """
...
class WriteRelOutputMode(betterproto.Enum):
    ...
    """
    this mode makes the operator return all the record INSERTED/DELETED/UPDATED
    by the operator. The operator returns the AFTER-image of any change. This
    can be further manipulated by operators upstreams (e.g., retunring the
    typical "count of modified records"). For scenarios in which the BEFORE
    image is required, the user must implement a spool (via references to
...
class ComparisonJoinKeySimpleComparisonType(betterproto.Enum):
    """
    Most joins will use one of the following behaviors.  To avoid the
    complexity of a function lookup we define the common behaviors here
    """
    ...
    """
    Returns true if both values are equal and not null Returns true if both
    values are null Returns false if one value is null and the other value is
    not null This can be expressed as a = b OR (isnull(a) AND isnull(b))
    """

    SIMPLE_COMPARISON_TYPE_MIGHT_EQUAL = 3
    """
    Returns true if both values are equal and not null Returns true if either
    value is null This can be expressed as a = b OR isnull(a = b)
    """
...
class ExpressionWindowFunctionBoundsType(betterproto.Enum):
    ...
    current row the window should extend.
    """

    BOUNDS_TYPE_RANGE = 2
    """
    The lower and upper bound describe a range of values.  The window should
    include all rows where the value of the ordering column is greater than or
    equal to (current_value - lower bound) and less than or equal to
    (current_value + upper bound).  This bounds type is only valid if there is
    a single ordering column.
    """
...
class FunctionSignatureFinalArgVariadicParameterConsistency(betterproto.Enum):
    ...
    """
    Each argument can be any possible concrete type afforded by the bounds of
    any parameter defined in the arguments specification.
    """
...
class Type(betterproto.Message):
    ...
    variations to be specified. If user_defined_type_reference is encountered,
...
class NamedStruct(betterproto.Message):
    """
    A message for modeling name/type pairs. Useful for representing relation
    schemas. Notes: * The names field is in depth-first order. For example a
    schema such as: a: int64 b: struct<c: float32, d: string> would have a
    `names` field that looks like: ["a", "b", "c", "d"] * Only struct fields
    are contained in this field's elements, * Map keys should be traversed
    first, then values when producing/consuming
    """
...
class RelCommon(betterproto.Message):
    ...
    """
    The underlying relation is output as is (no reordering or projection of
    columns)
    """
...
class ReadRel(betterproto.Message):
    """
    The scan operator of base data (physical or virtual), including filtering
    and projection.
    """
...
class ReadRelNamedTable(betterproto.Message):
    ...
    mydb.mytable). This assumes shared catalog between systems exchanging a
    message.
    """
...
class ReadRelLocalFilesFileOrFiles(betterproto.Message):
    """
    Many files consist of indivisible chunks (e.g. parquet row groups or CSV
    rows).  If a slice partially selects an indivisible chunk then the consumer
    should employ some rule to decide which slice to include the chunk in (e.g.
    include it in the slice that contains the midpoint of the chunk)
    """
    ...
    """
    A URI where the path portion is a glob expression that can identify zero or
    more paths. Consumers should support the POSIX syntax.  The recursive
...
class ProjectRel(betterproto.Message):
    """
    This operator allows to represent calculated expressions of fields (e.g.,
    a+b). Direct/Emit are used to represent classical relational projections
    """
...
class JoinRel(betterproto.Message):
    ...
    types, a join condition and post_join_filter expression
    """
...
class FetchRel(betterproto.Message):
    ...
    """
    the amount of records to return use -1 to signal that ALL records should be
    returned
    """
...
class AggregateRel(betterproto.Message):
    ...
    """
    A list of one or more grouping expression sets that the aggregation
    measures should be calculated for. Required if there are no measures.
    """
...
class AggregateRelMeasure(betterproto.Message):
    ...
    within the measure. Helps to support SUM(<c>) FILTER(WHERE...) syntax
    without masking opportunities for optimization
    """
...
class ConsistentPartitionWindowRel(betterproto.Message):
    """
    ConsistentPartitionWindowRel provides the ability to perform calculations
    across sets of rows that are related to the current query row. It can be
    used to execute window functions where all the windows share the same
...
class ConsistentPartitionWindowRelWindowRelFunction(betterproto.Message):
    ...
    defining the partition, sorts, and bounds, since those must be consistent
    across the various functions in this rel.  Refer to the `WindowFunction`
...
class SortRel(betterproto.Message):
    """
    The ORDERY BY (or sorting) relational operator. Beside describing a base
    relation, it includes a list of fields to sort on
    """
...
class SetRel(betterproto.Message):
    ...
    """
    The first input is the primary input, the remaining are secondary inputs.
...
class ExpandRel(betterproto.Message):
    """
    Duplicates records by emitting one or more rows per input row.  The number
    of rows emitted per input row is the same for all input rows. In addition
    to a field being emitted per input field an extra int64 field is emitted
    which contains a zero-indexed ordinal corresponding to the duplicate
    definition.
    """
    ...
    """
    There should be one definition here for each input field.  Any fields
    beyond the provided definitions will be emitted as is (as if a
    consistent_field record with an identity expression was provided).
    """
...
class ExpandRelExpandField(betterproto.Message):
    ...
    switching_field should contain the same number of duplicates (so that the
    output rows are of consistent size and type).  If there are not enough
    switching field definitions to match the other field definitions NULL will
    be returned to fill the extras.
    """

    consistent_field: "Expression" = betterproto.message_field(3, group="field_type")
    """
    Field that outputs the same value no matter which duplicate is being
    output.  Equivalent to a switching_field that lists the same expression
    multiple times.
    """
...
class ExpandRelSwitchingField(betterproto.Message):
    ...
    """
    All duplicates must return the same type class but may differ in
    nullability.  The effective type of the output field will be nullable if
    any of the duplicate expressions are nullable.
    """
...
class RelRoot(betterproto.Message):
    """
    A relation with output field names. This is for use at the root of a `Rel`
    tree.
    """
...
class DdlRel(betterproto.Message):
    ...
    """
    The columns that will be modified (representing after-image of a schema
    change)
    """

    table_defaults: "ExpressionLiteralStruct" = betterproto.message_field(4)
    """
    The default values for the columns (representing after-image of a schema
    change) E.g., in case of an ALTER TABLE that changes some of the column
    default values, we expect the table_defaults Struct to report a full list
    of default values reflecting the result of applying the ALTER TABLE
...
class WriteRel(betterproto.Message):
    """
    The operator that modifies the content of a database (operates on 1 table
    at a time, but record-selection/source can be based on joining of multiple
    tables).
    """
    ...
    table_schema: "NamedStruct" = betterproto.message_field(3)
    """
    The schema of the table (must align with Rel input (e.g., number of leaf
    fields must match))
    """
    ...
    """
    The relation that determines the records to add/remove/modify the schema
    must match with table_schema. Default values must be explicitly stated in a
    ProjectRel at the top of the input. The match must also occur in case of
    DELETE to ensure multi-engine plans are unequivocal.
    """
...
class ComparisonJoinKey(betterproto.Message):
    """
    Hash joins and merge joins are a specialization of the general join where
    the join expression is an series of comparisons between fields that are
    ANDed together.  The behavior of this comparison is flexible
    """
...
class ComparisonJoinKeyComparisonType(betterproto.Message):
    ...
    """
    A custom comparison behavior is used.  This can happen, for example, when
    using collations, where we might want to do something like a case-
    insensitive comparison. This must be a binary function with a boolean
    return type
    """
...
class HashJoinRel(betterproto.Message):
    ...
    input based on a set of join keys. It will then probe that hash table for
    incoming inputs, finding matches. Two rows are a match if the comparison
    function returns true for all keys
    """
    ...
    """
    These fields are deprecated in favor of `keys`.  If they are set then the
    two lists (left_keys and right_keys) must have the same length and the
    comparion function is considered to be SimpleEqualityType::EQ
    """
    ...
    (unless the deprecated left_keys/right_keys fields are being used). If a
    custom comparison function is used then it must be consistent with the hash
    function used for the keys. In other words, the hash function must return
    the same hash code when the comparison returns true.  For example, if the
    comparison function is "equals ignoring case" then the hash function must
    return the same hash code for strings that differ only by case.  Note: the
    hash function is not specified here.  It is the responsibility of the
    consumer to find an appropriate hash function for a given comparsion
    function or to reject the plan if it cannot do so.
    """
...
class MergeJoinRel(betterproto.Message):
    ...
    One or more keys to join on.  The relation is invalid if this is empty
    (unless the deprecated left_keys/right_keys fields are being used). If a
    custom comparison function is used then it must be consistent with the
    ordering of the input data.  For example, if the comparison function is "<"
    then we generally expect the data to be sorted in ascending order. If the
    comparison function is something like "less than ignoring case" then the
    data should be sorted appropriately (e.g. both "A" and "a" should come
    before "b") The sort order is not specified here.  It is typically the
    responsibility of the producer to ensure the plan sorts the data if needed
    (although the consumer is free to do so as well).  If possible, the
    consumer should verify the sort order and reject invalid plans.
    """
...
class NestedLoopJoinRel(betterproto.Message):
    ...
    iterate over it using the left input, evaluating the join expression on the
...
class FunctionOption(betterproto.Message):
    ...
    """
    Name of the option to set. If the consumer does not recognize the option,
    it must reject the plan. The name is matched case-insensitively with option
    names defined for the function.
    """
    ...
    supports. If the consumer supports none of the specified options, it must
    reject the plan. The name is matched case-insensitively and must match one
    of the option values defined for the option.
    """
...
class Expression(betterproto.Message):
    ...
    """
    deprecated: enum literals are only sensible in the context of function
    arguments, for which FunctionArgument should now be used
    """
...
class ExpressionLiteral(betterproto.Message):
    ...
    """
    Timestamp in units of microseconds since the UNIX epoch. Deprecated in
    favor of `precision_timestamp`
    """
    ...
    """
    If the precision is 6 or less then this is the microseconds since the UNIX
    epoch If the precision is more than 6 then this is the nanoseconds since
    the UNIX epoch
    """
    ...
    """
    Timestamp in units of microseconds since the UNIX epoch. Deprecated in
    favor of `precision_timestamp_tz`
    """
    ...
    Applies to all members of the literal_type oneof EXCEPT:  * Type null
    (must be nullable by definition)  * Type.List empty_list  (use
    Type.List::nullability)  * Type.Map empty_map    (use
    Type.Map::nullability)
    """

    type_variation_reference: int = betterproto.uint32_field(51)
    """
    optionally points to a type_variation_anchor defined in this plan. Applies
    to all members of union other than the Typed null (which should directly
    declare the type variation).
    """
...
class ExpressionLiteralDecimal(betterproto.Message):
    ...
    precision: int = betterproto.int32_field(2)
    """
    The maximum number of digits allowed in the value. the maximum precision is
    38.
    """
...
class ExpressionLiteralUserDefined(betterproto.Message):
    ...
    """
    the value of the literal, serialized using some type-specific protobuf
    message
    """

    struct: "ExpressionLiteralStruct" = betterproto.message_field(4, group="val")
    """
    the value of the literal, serialized using the structure definition in its
    declaration
    """
...
class ExpressionNested(betterproto.Message):
    ...
    type_variation_reference: int = betterproto.uint32_field(2)
    """
    Optionally points to a type_variation_anchor defined in this plan for the
    returned nested type.
    """
...
class ExpressionNestedMap(betterproto.Message):
    ...
    """
    One or more key-value pairs. To specify an empty map, use Literal.empty_map
...
class ExpressionNestedStruct(betterproto.Message):
    ...
    """
    Zero or more possibly heterogeneously-typed list of expressions that form
    the struct fields.
    """
...
class ExpressionNestedList(betterproto.Message):
    ...
    """
    A homogeneously-typed list of one or more expressions that form the list
    entries. To specify an empty list, use Literal.empty_list (otherwise type
...
class ExpressionScalarFunction(betterproto.Message):
    ...
    function_reference: int = betterproto.uint32_field(1)
    """
    Points to a function_anchor defined in this plan, which must refer to a
    scalar function in the associated YAML file. Required; avoid using
    ...
    types must also match exactly:  - Value arguments must be bound using
    FunctionArgument.value, and    the expression in that must yield a value of
    a type that a function    overload is defined for.  - Type arguments must
    be bound using FunctionArgument.type.  - Enum arguments must be bound using
    FunctionArgument.enum    followed by Enum.specified, with a string that
    case-insensitively    matches one of the allowed options.
    """
    ...
    if the consumer does not need specific behavior in these cases.
    """
...
class ExpressionWindowFunction(betterproto.Message):
    ...
    a window function  - an aggregate function An aggregate function referenced
    here should be treated as a window function with Window Type STREAMING
    ...
    FunctionArgument.value, and    the expression in that must yield a value of
    a type that a function    overload is defined for.  - Type arguments must
    be bound using FunctionArgument.type, and a    function overload must be
    defined for that type.  - Enum arguments must be bound using
    FunctionArgument.enum    followed by Enum.specified, with a string that
    case-insensitively    matches one of the allowed options.
    """
    ...
    phase: "AggregationPhase" = betterproto.enum_field(6)
    """
    Describes which part of the window function to perform within the context
    of distributed algorithms. Required. Must be set to INITIAL_TO_RESULT for
    window functions that are not decomposable.
    """

    sorts: List["SortField"] = betterproto.message_field(3)
    """
    If specified, the records that are part of the window defined by
    upper_bound and lower_bound are ordered according to this list before they
    are aggregated. The first sort field has the highest priority; only if a
    sort field determines two records to be equivalent is the next field
    queried. This field is optional, and is only allowed if the window function
    is defined to support sorting.
    """
    ...
    function, only the subset of records within the bounds that are also in the
    same partition as the current record are aggregated.
    """
    ...
    greater than the upper bound, TODO (null range/no records passed? wrapping
    around as if lower/upper were swapped? error? null?). Optional; defaults to
    ...
    extends. The bound is inclusive. If the upper bound indexes a record less
    than the lower bound, TODO (null range/no records passed? wrapping around
    as if lower/upper were swapped? error? null?). Optional; defaults to the
...
class ExpressionWindowFunctionBound(betterproto.Message):
    ...
    partition, depending on whether this represents the upper or lower bound.
    """
...
class ExpressionWindowFunctionBoundPreceding(betterproto.Message):
    ...
    """
    A strictly positive integer specifying the number of records that the
    window extends back from the current record. Required. Use CurrentRow for
    offset zero and Following for negative offsets.
    """
...
class ExpressionWindowFunctionBoundFollowing(betterproto.Message):
    ...
    """
    A strictly positive integer specifying the number of records that the
    window extends ahead of the current record. Required. Use CurrentRow for
    offset zero and Preceding for negative offsets.
    """
...
class ExpressionWindowFunctionBoundUnbounded(betterproto.Message):
    """
    Defines an "unbounded bound": for lower bounds this means the start of the
    partition, and for upper bounds this means the end of the partition.
    """
...
class ExpressionReferenceSegment(betterproto.Message):
    ...
    either a map key by literal, a struct field by the ordinal position of the
    desired field or a particular element in an array. Supports expressions
    that would roughly translate to something similar to:
    a.b[2].c['my_map_key'].x where a,b,c and x are struct field references
    (ordinalized in the internal representation here), [2] is a list offset and
    ['my_map_key'] is a reference into a map field.
    """
...
class ExpressionMaskExpression(betterproto.Message):
    ...
    pipeline. Note that this does not fundamentally alter the structure of data
...
class ExpressionSubqueryScalar(betterproto.Message):
    """
    A subquery with one row and one column. This is often an aggregate though
    not required to be.
    """
...
class ExpressionSubqueryInPredicate(betterproto.Message):
    ...
    subquery Examples: x IN (SELECT * FROM t) (x, y) IN (SELECT a, b FROM t)
    """
...
class ExpressionSubquerySetPredicate(betterproto.Message):
    """
    A predicate over a set of rows in the form of a subquery EXISTS and UNIQUE
    are common SQL forms of this operation.
    """
...
class ExpressionSubquerySetComparison(betterproto.Message):
    """
    A subquery comparison using ANY or ALL. Examples: SELECT * FROM t1 WHERE x
    < ANY(SELECT y from t2)
    """
...
class SortField(betterproto.Message):
    """
    The description of a field to sort on (including the direction of sorting
    and null semantics)
    """
...
class AggregateFunction(betterproto.Message):
    ...
    function_reference: int = betterproto.uint32_field(1)
    """
    Points to a function_anchor defined in this plan, which must refer to an
    aggregate function in the associated YAML file. Required; 0 is considered
    to be a valid anchor/reference.
    """
    ...
    defined for that type.  - Enum arguments must be bound using
    FunctionArgument.enum    followed by Enum.specified, with a string that
    case-insensitively    matches one of the allowed options.  - Optional enum
    arguments must be bound using FunctionArgument.enum    followed by either
    Enum.specified or Enum.unspecified. If specified,    the string must case-
    insensitively match one of the allowed options.
    """
    ...
    before they are aggregated. The first sort field has the highest priority;
    only if a sort field determines two records to be equivalent is the next
    field queried. This field is optional.
    """
...
class ReferenceRel(betterproto.Message):
    """
    This rel is used  to create references, in case we refer to a RelRoot field
...
class Plan(betterproto.Message):
    """
    Describe a set of operations to complete. For compactness sake, identifiers
    are normalized at the plan level.
    """
...
class PlanVersion(betterproto.Message):
    """
    This message type can be used to deserialize only the version of a
    Substrait Plan message. This prevents deserialization errors when there
    were breaking changes between the Substrait version of the tool that
    produced the plan and the Substrait version used to deserialize it, such
    that a consumer can emit a more helpful error message in this case.
    """
...
class Version(betterproto.Message):
    ...
    """
    If a particular version of Substrait is used that does not correspond to a
    version number exactly (for example when using an unofficial fork or using
    a version that is not yet released or is between versions), set this to the
    full git hash of the utilized commit of https://github.com/substrait-
    io/substrait (or fork thereof), represented using a lowercase hex ASCII
    string 40 characters in length. The version number above should be set to
    the most recent version tag in the history of that commit.
    """
...
class Capabilities(betterproto.Message):
    """
    Defines a set of Capabilities that a system (producer or consumer)
    supports.
    """
...
class ParameterizedType(betterproto.Message):
    ...
    timestamp: "TypeTimestamp" = betterproto.message_field(14, group="kind")
    """
    Deprecated in favor of `ParameterizedPrecisionTimestamp
    precision_timestamp`
    """
    ...
    timestamp_tz: "TypeTimestampTz" = betterproto.message_field(29, group="kind")
    """
    Deprecated in favor of `ParameterizedPrecisionTimestampTZ
    ...
    """
    Deprecated in favor of user_defined, which allows nullability and
    variations to be specified. If user_defined_pointer is encountered, treat
    it as being non-nullable and having the default variation.
    """
...
class DerivationExpression(betterproto.Message):
    ...
    timestamp_tz: "TypeTimestampTz" = betterproto.message_field(29, group="kind")
    """
    Deprecated in favor of `ExpressionPrecisionTimestampTZ
    precision_timestamp_tz`
    """
