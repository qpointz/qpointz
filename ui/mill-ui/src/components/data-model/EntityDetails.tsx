...
export function EntityDetails({ entity, facets }: EntityDetailsProps) {
  ...
  const hasDescriptive = facets.descriptive && Object.keys(facets.descriptive).length > 0;
  const hasStructural = facets.structural && Object.keys(facets.structural).length > 0;
  const hasRelations = facets.relations && facets.relations.length > 0;
